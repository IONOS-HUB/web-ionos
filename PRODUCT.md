...
3. **Marketing** — pauta digital, cobertura de eventos, creación de contenido digital, optimización de campañas, creación de conjuntos de anuncios, landing pages a medida, y todo el entorno de Meta. Incluye **SEO** (posicionamiento en buscadores) y **AEO** (optimización para motores de respuesta: que ChatGPT, Gemini, Perplexity y Claude nombren al cliente). La prueba es el propio sitio de IonosHub: datos estructurados, `llms.txt`, sitemap indexado y rastreadores de IA permitidos en `robots.txt`.
...
- **Backend:** una API route de Astro en Vercel (`src/pages/api/lead.ts`) que valida el lead y lo reenvía por POST al webhook de n8n (`LEAD_WEBHOOK_URL`), donde se registra en el CRM y se notifica al equipo. Resend quedó fuera: no se usa correo transaccional. La URL del webhook vive en variable de entorno, nunca en el código.
