...
### Galería de trabajo real (`MediaGallery`, ventanilla de fotos y capturas)
Las únicas ventanillas con material real, nunca demostrativo: los datos viven en `src/data/photos.ts` (`coverageGallery`, `landingsGallery`, `systemsGallery`). Misma anatomía que las demás (cabecera cobalto con disco `image` o `software`, cuerpo blanco, pie `ice-3`, radio 1.5 rem, borde `line`, sombra `lift`), pero el badge dice "Fotos reales" o "Capturas reales", nunca "Demostración". Cada pieza es un botón `tile` de radio 0.75 rem con borde `line` y fondo `ice-2` (el "papel"), cursor `zoom-in`.
- **Variante foto** (cobertura de eventos): mosaico `grid-cols-2` (móvil) / `grid-cols-4 grid-rows-2` (`sm`) con la primera foto a doble celda (`lead`).
- **Variante pantalla** (landing pages, sistemas): cada captura va en un marco de navegador en miniatura: barra de 1.5 rem `ice-3` con borde `line-soft`, tres puntos `line` de 0.4 rem y el nombre del cliente en una píldora `canvas` de 0.62rem/600 `ink-3`; la captura 16:9 recortada por arriba (`object-position: top`). Retícula 1 / 2 (`sm`) / 3 (`lg`) columnas; con cuatro piezas se queda en 2×2.
- Sobre la primera pieza, si el servicio lo promete, la tarjeta de entrega ("Entrega el mismo día · Editadas y listas para tus redes"; "Software a medida · Alcance cerrado y entrega en 30 días") con la anatomía de la tarjeta "Cita agendada" (`ice-3/95` con desenfoque de fondo, borde `ice-2`, disco cobalto). Pie: caption del servicio y "N fotos/landings/sistemas · clic para ampliar" en `tnum`.
- La variante `compact` (home, bajo la fila del pilar) es una fila sin ventanilla: cinco tiles 4:5 (tres en móvil) para fotos, cuatro marcos 16:9 (dos en móvil) para pantallas.
- Visor: `<dialog>` con fondo `night/90`, imagen a `min(92vw, 72rem)` × 76vh con radio 1.5 rem, borde `white/20` y sombra `phone`; pie con el texto alternativo en `night-text` y contador `tnum` blanco; botones en discos de 3 rem `white/12` con borde `white/25` (cerrar arriba a la derecha, anterior/siguiente a media altura; en móvil bajan a la base). Teclado: flechas y Escape; táctil: deslizar 48 px. Una instancia de visor por galería. El contenedor padre de la ventanilla debe llevar `min-w-0` si es ítem de grid.

### Placa de logo de cliente (`ClientLogo`)
Los logos de clientes llegan en lienzos cuadrados con la marca ocupando el 60–90 %; se recortan al trazo con margen uniforme (`public/imgs/clientes/marca/*.webp`, 240 px de alto) y se muestran siempre dentro de una placa blanca de proporción fija donde la marca se ajusta al máximo (`object-contain`). Así todas pesan igual, también sobre noche. Tamaños: `sm` 3 × 5.5 rem, `md` 4 × 7 rem, `lg` 5 × 9 rem, `xl` 6 × 11 rem; borde 1 px `line`, radio 0.75–1.25 rem según tamaño. Sobre `ice-3` (franja de logos) la placa pasa a `ice-3`/`line-soft`. Nunca un logo suelto a altura fija.

### Casos (banda noche y "Lo que ya logramos")
- **Home:** un caso destacado en panel `night-2` (7 columnas): placa `lg` + cliente/industria + chip del pilar, cifra `metric` a `clamp(3.4rem, 6vw, 5.5rem)` con el "antes" tachado en `night-text`, y Reto/Solución/Resultado en tres columnas bajo una línea `night-line`. A su lado (5 columnas), lista de casos en filas divididas por `night-line`: placa `md`, cifra 1.5rem, cliente · industria, pilar en caption; la fila entera enlaza a `/casos-de-exito` y se enciende a `night` en hover.
- **Página de pilar:** filas editoriales divididas por `night-line` (5/7): placa `lg` + cliente/industria y cifra `metric` a la izquierda; Reto/Solución/Resultado en tres columnas a la derecha. Un solo caso ocupa toda la fila; nunca una tarjeta huérfana en una retícula de tres.

### Ventanilla de caso real (`AgentStory`)
Recrea paso a paso un flujo real de un cliente (el agente de IA de Daniela Vidal): misma anatomía de ventanilla, badge "Caso real", cabecera de chat con nombre del lead y canal · hora, transcripción con caption de hablante ("Lead" / "Agente de IA"), fila de chips (`chip-on` "Lead calificado" + chip de clasificación), dos tarjetas de acción con disco cobalto (`database` "Registrado en el CRM unificado", `users` "Daniela entra sólo a cerrar y cobrar") y pie "Flujo real · conversación recreada" / "Datos ficticios". La honestidad va en la interfaz: el flujo es real, los datos no.
...
- **Revelado y entrega** (galería de cobertura): las fotos entran como copias que se revelan, no como bloques que suben. Cada `tile` reutiliza el observador del reveal (`data-reveal`, escalones de 140 ms) pero sustituye la coreografía: `clip-path: inset(0 0 100% 0)` → `inset(0)` en 0.9 s, `blur(16px) saturate(0.1) brightness(1.2)` → nítido en 1.2 s y `scale(1.1)` → 1 en 1.3 s, todo ease-out-expo; a los 900 ms salta la tarjeta "Entrega el mismo día" (12 px + `scale(0.96)` → 1). Hover: la foto crece a 1.04 en 0.7 s y un barrido de luz diagonal (`white/28`) cruza la copia en 0.9 s, sólo en punteros con hover. La foto principal deriva ±5 % con el scroll mediante `animation-timeline: view()` bajo `@supports`, sin JS. Visor: zoom continuo FLIP desde la miniatura (WAAPI, 560 ms ease-out-expo, radio 0.75 → 1.5 rem) y regreso a ella al cerrar (380 ms); cambio de foto por deslizamiento de 28 px + fade en 420 ms; fondo con `lb-fade` 0.4 s. Con `prefers-reduced-motion` el mosaico aparece revelado y el visor abre y cierra sin transformar.
- **Secuencia de caso** (`AgentStory`): una sola pasada disparada por el reveal de la ventanilla; cada paso (`.step`) entra con 12 px + fade en 0.6 s ease-out-expo a su tiempo (`--d`, de 0.3 s a 8.7 s: lead → agente → lead → agente → lead → chips → CRM → cierre). Se escribe con longhands (`animation-name/duration/timing-function/fill-mode/delay`): el minificador vacía el atajo `animation` cuando combina `var()` con un `animation-delay` aparte.
- **Freno:** `prefers-reduced-motion` y `?nomotion=1` desactivan todo y muestran el estado final (todos los pasos activos, bandeja con las cuatro conversaciones resueltas, chips encendidos, galería revelada, secuencia de caso completa).
